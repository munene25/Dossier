...
class ResumeTests(TestCase):
    ...
    def create_dummy_pdf(content_type):
        return SimpleUploadedFile(
            "test_resume.pdf", b"dummy_content", content_type=content_type
        )

    def test_resume_form_with_valid_file(self):
        """test if the form returns true for a valid file format"""
        form = ResumeForm(
            data={}, files={"pdf_file": self.create_dummy_pdf("application/pdf")}
        )
    ...
    def test_resume_form_with_invalid_file(self):
        ...
        form = ResumeForm(
            data={}, files={"pdf_file": self.create_dummy_pdf("text/plain")}
        )  # Use the stored file
    ...
    def test_upload_valid_pdf(self):
        ...
        pdf_path = os.path.join(
            settings.BASE_DIR, "resumes", "tests", "test_resume.pdf"
        )
